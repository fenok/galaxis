...
interface QueryOptions<C extends SDC = any, R extends RC = any, P extends PPC = any, Q extends QPC = any, B extends BC = any> {
    getPartialRequestId?(request: PartialRequestData<C, R, P, Q, B>): string | number;
}

export function useQuery<C extends SDC = any, R extends RC = any, P extends PPC = any, Q extends QPC = any, B extends BC = any>(request: PartialRequestData<C, R, P, Q, B>, {getPartialRequestId}: QueryOptions = {}) {
    ...
    const query = React.useCallback((forceUpdate?: boolean) => {
        ...
        return client.query(request, {callerId: componentId, forceNetworkRequest: !!forceUpdate, multiAbortSignal: multiAbortControllerRef.current.signal})
    }, [client, requestId, componentId]);
